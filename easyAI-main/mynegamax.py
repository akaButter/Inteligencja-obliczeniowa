...
def negamax(game, depth, origDepth, scoring, alpha=+inf, beta=-inf, tt=None, pruning = True):
    """
    ...
    for move in possible_moves:
        ...
        if pruning:
            move_alpha = -negamax(game, depth - 1, origDepth, scoring, -beta, -alpha, tt = tt, pruning = pruning)
        else:
            move_alpha = -negamax(game, depth - 1, origDepth, scoring, tt = tt, pruning = pruning)
        if unmake_move:
            game.switch_player()
