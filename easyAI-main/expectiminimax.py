...
inf = float("infinity")
def expectiminimax(game, depth, origDepth, scoring, alpha=+inf, beta=-inf, pruning= True):
    ...
    for move in possible_moves:
        ...
        if pruning:
            ...
            move_alpha_no = -expectiminimax(game, depth - 1, origDepth, scoring, pruning = pruning)
        if unmake_move:
            game.switch_player()
            game.unmake_move(move)
        if game.player.lost_pawns:
            if not unmake_move:
                game = state.copy()
            game.make_move(move, force_resurrect = True)
            game.switch_player()
            move_alpha_yes = -expectiminimax(game, depth-1, origDepth, scoring, pruning=pruning)
            if unmake_move:
                game.switch_player()
                game.unmake_move(move)
